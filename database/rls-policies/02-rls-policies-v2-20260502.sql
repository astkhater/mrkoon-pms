-- =================================================================
-- Mrkoon OKR Webapp — RLS Policies v2
-- Date: 2026-05-02
-- Run AFTER 01-schema-v2.sql.
--
-- Changes from v1:
--   + config.levels        (Admin RW; everyone reads — needed for org chart)
--   + config.assumptions   (Admin/HR/Finance RW; dept head + manager read own dept; employee read own role)
--   * track.appraisals.goals_score column inherits existing appraisal policies
-- All v1 policies otherwise carried over verbatim.
-- =================================================================
...
create or replace function auth.user_functional_role() returns uuid
language sql stable security definer as $$
...
alter table config.levels              enable row level security;       -- NEW v2
alter table config.assumptions         enable row level security;       -- NEW v2
...
-- CONFIG SCHEMA
-- =================================================================
create policy config_rates_admin_finance on config.compensation_rates for all using (auth.is_admin() or auth.is_finance()) with check (auth.is_admin() or auth.is_finance());
create policy config_rates_read_finance_admin on config.compensation_rates for select using (auth.is_admin() or auth.is_finance());

create policy config_targets_admin on config.targets for all using (auth.is_admin()) with check (auth.is_admin());
create policy config_targets_read on config.targets for select using (true);

create policy config_thresholds_admin on config.thresholds for all using (auth.is_admin()) with check (auth.is_admin());
create policy config_thresholds_read on config.thresholds for select using (true);

create policy config_salary_bands_admin_finance on config.salary_bands for all using (auth.is_admin() or auth.is_finance()) with check (auth.is_admin() or auth.is_finance());
create policy config_salary_bands_read_finance_admin on config.salary_bands for select using (auth.is_admin() or auth.is_finance());

-- NEW v2: levels — Admin/HR write; everyone reads (needed for org chart, sidebar, role display)
create policy config_levels_admin_hr on config.levels for all using (auth.is_admin() or auth.is_hr()) with check (auth.is_admin() or auth.is_hr());
create policy config_levels_read on config.levels for select using (true);

-- NEW v2: assumptions — Admin/HR/Finance RW; dept head reads own dept; manager reads own dept; employee reads own functional role
create policy config_assumptions_admin_hr_finance on config.assumptions for all
  using (auth.is_admin() or auth.is_hr() or auth.is_finance())
  with check (auth.is_admin() or auth.is_hr() or auth.is_finance());
create policy config_assumptions_dept_head_read on config.assumptions for select using (
  auth.is_dept_head() and (department_code is null or department_code = (select code from def.departments where id = auth.user_dept()))
);
create policy config_assumptions_manager_read on config.assumptions for select using (
  auth.is_manager() and (department_code is null or department_code = (select code from def.departments where id = auth.user_dept()))
);
create policy config_assumptions_employee_read on config.assumptions for select using (
  auth.is_employee() and (
    functional_role_code is null
    or functional_role_code = (select code from def.functional_roles where id = auth.user_functional_role())
  )
);
create policy config_assumptions_clevel_read on config.assumptions for select using (auth.is_clevel());

create policy config_rating_bands_admin_hr on config.rating_bands for all using (auth.is_admin() or auth.is_hr()) with check (auth.is_admin() or auth.is_hr());
create policy config_rating_bands_read on config.rating_bands for select using (true);

create policy config_periods_admin_hr on config.cycle_periods for all using (auth.is_admin() or auth.is_hr()) with check (auth.is_admin() or auth.is_hr());
create policy config_periods_read on config.cycle_periods for select using (true);

-- =================================================================
-- DEF SCHEMA (carried from v1)
-- =================================================================
...
create policy def_users_dept_head on def.users for select using (auth.is_dept_head() and department_id = auth.user_dept());
create policy def_users_hr_finance_clevel_admin on def.users for select using (auth.is_admin() or auth.is_hr() or auth.is_finance() or auth.is_clevel());
create policy def_users_admin_hr_write on def.users for all using (auth.is_admin() or auth.is_hr()) with check (auth.is_admin() or auth.is_hr());
...
-- =================================================================
-- TRACK SCHEMA (carried from v1; goals_score column inherits)
-- =================================================================
...
create policy track_actuals_finance_scheme on track.kpi_actuals for select using (auth.is_finance() and exists (select 1 from def.kpis k where k.id = kpi_actuals.kpi_id and k.scheme_ref is not null));
...
create policy track_apps_dept_head on track.appraisals for all using (auth.is_dept_head() and auth.in_my_dept(employee_id)) with check (auth.is_dept_head() and auth.in_my_dept(employee_id));
...
-- No INSERT/UPDATE/DELETE policy → only security definer trigger function writes; immutable.
